      ...
      <div className="max-w-md w-full space-y-6">
        ...
        <GlassCard className="space-y-6 border-amber-500/30 p-6">
          <motion.div
            ...
            className="w-20 h-20 rounded-full bg-amber-500/10 border border-amber-500/30 flex items-center justify-center mx-auto text-amber-500 dark:text-amber-400 relative"
          >
          ...
          <div className="space-y-2">
            ...
            <h2 className="text-xl font-extrabold text-slate-800 dark:text-white tracking-tight pt-2">
              Menunggu Persetujuan Admin
            </h2>
            <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed px-2">
              Data pendaftaran Anda telah berhasil dikirim ke sistem <strong className="text-blue-600 dark:text-blue-400">AzurLizeTeam</strong>. Silakan tunggu hingga Admin menyetujui akun Anda.
            </p>
          </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-900/80 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 text-left space-y-2 text-xs">
            ...
            <div className="space-y-1 text-slate-500 dark:text-slate-400">
              <p>📍 Nama: <span className="text-slate-800 dark:text-white font-medium">{userProfile?.firstName} {userProfile?.lastName}</span></p>
              <p>📍 Email: <span className="text-slate-800 dark:text-white font-medium">{userProfile?.email}</span></p>
              <p>📍 WhatsApp: <span className="text-slate-800 dark:text-white font-medium">{userProfile?.whatsapp}</span></p>
              <p>📍 UID 9Kucing: <span className="text-slate-800 dark:text-white font-medium">{userProfile?.akun9Kucing}</span></p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center gap-2 text-[11px] text-slate-400 dark:text-slate-500">
            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 dark:text-emerald-400" /> Auto-sync aktif. Halaman akan otomatis masuk ketika disetujui.
          </div>
        </GlassCard>
      </div>
    </div>
